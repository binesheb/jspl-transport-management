...
void saveSettings(){if(!pinOK()){server.send(403,"text/plain","Invalid administrator PIN");return;}DeviceConfig c=settings();c.deviceId=clean(server.arg("deviceId"),31);c.deviceName=clean(server.arg("deviceName"),40);c.showroom=clean(server.arg("showroom"),40);c.installation=clean(server.arg("installation"),40);c.busRegistration=clean(server.arg("busReg"),24);c.busCapacity=constrain(server.arg("busCap").toInt(),1,100);for(uint8_t i=0;i<3;i++){c.destinationCode[i]=clean(server.arg("c"+String(i)),8);c.destinationName[i]=clean(server.arg("n"+String(i)),24);}c.wifiSsid=clean(server.arg("ssid"),64);c.wifiPassword=clean(server.arg("pass"),64);settingsSave(c);WiFi.disconnect();if(c.wifiSsid.length())WiFi.begin(c.wifiSsid.c_str(),c.wifiPassword.c_str());server.send(200,"text/html",settingsPage("Saved. The dashboard is available at the local AP address shown on Serial Monitor."));screen();}
void webReset(){if(!pinOK()){server.send(403,"text/plain","Invalid administrator PIN");return;}int i=server.arg("index").toInt();if(i<0||i>=3){server.send(400,"text/plain","Invalid counter");return;}clearCounter(i);server.send(200,"text/plain",name(i)+" counter reset to zero.");}
